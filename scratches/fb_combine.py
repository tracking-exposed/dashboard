...
p.add('--start', help='start date for harmonizer. default is all data',
      default=str(datetime.date.today() - datetime.timedelta(days=99999)))
p.add('-e', '--end', help='end date for harmonizer. default is all data.', default=str(datetime.date.today()))
p.add('--sources', help='MUST BE A DIRECTORY containing ONLY csv files from FBcrawl, used to merge with user data',
      required=True)
p.add('--users', help='MUST BE A DIRECTORY containing ONLY csv files from fbTREX, used to merge with user data',
      required=True)
...
def main():
    # Data Loading
    fbtrex_output_files = [os.path.join(config['users'], f) for f in os.listdir(config['users']) if
                           os.path.isfile(os.path.join(config['users'], f)) and f.endswith(".csv")]
    ...
    fbcrawl_output_files = [os.path.join(config['sources'], f) for f in os.listdir(config['sources']) if
                            os.path.isfile(os.path.join(config['sources'], f)) and f.endswith(".csv")]
    ...
    user_data = user_data.drop(['ANGRY', 'HAHA', 'LIKE', 'LOVE', 'SAD', 'WOW', 'displaySource',
                                ...
                                'semanticId', 'sourceLink', 'texts', 'textsize', 'timeline'], axis=1).drop_duplicates(
        ['id']).fillna(0)
